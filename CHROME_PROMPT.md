# Claude in Chrome 用 設定プロンプト

Claude in Chrome（Chrome 拡張の Claude）に、スプレッドシート版の設定作業を代わりにやってもらうためのプロンプトです。

## 使い方

1. Chrome で、**リマインドメールの送信元にしたい Google アカウント**と、**GitHub** の両方にログインしておきます。
2. Claude in Chrome のパネルを開き、下の「プロンプト」の枠の中をすべてコピーして貼り付け、送信します。
3. Google の「承認」画面や GitHub の保存確認など、**本人の操作が必要な場面では Claude が止まって声をかけます**。画面の指示に従って操作してから「続けて」と送ってください。

> GAS のコードは下の「GAS（Code.gs）」にも全文を載せています。Claude がコードのコピーに失敗したときは、ここから手動でコピーして Apps Script に貼り付けてください。
...
```text
あなたは私の代わりに、Google スプレッドシートと Google Apps Script（GAS）、GitHub Pages の設定作業を行うアシスタントです。
以下の手順を順番に実行してください。各手順が終わるたびに、何をしたかを1行で報告してから次へ進んでください。

【大事なルール】
- Google の権限の承認画面（「このアプリは Google で確認されていません」「アクセスを許可」など）が出たら、自分では押さずに作業を止め、「承認画面が出ました。内容を確認して許可してください。終わったら『続けて』と送ってください」と私に伝えてください。
- パスワードの入力や、アカウントのログインが必要になった場合も、同じように止めて私に任せてください。
- コードは手で打ち込まないでください。必ずコピー＆貼り付けで入れてください（エディタの自動補完で壊れるため）。
- 予想と違う画面が出たり、エラーが出たら、無理に進めずに画面の内容を私に報告してください。

【手順1：スプレッドシートを作る】
1. https://sheets.new を開いて、新しいスプレッドシートを作ってください。
2. 左上のファイル名「無題のスプレッドシート」を「青年委員 応援カレンダー」に変えてください。

【手順2：GAS のコードを貼り付ける】
1. スプレッドシートのメニューで「拡張機能」→「Apps Script」を開いてください（新しいタブで開きます）。
2. 別のタブで次のページを開いてください。
   https://github.com/lc100068-cmd/seinen-ouen/blob/main/gas/Code.gs
3. ファイル表示の右上にある「Copy raw file（生ファイルをコピー）」ボタンを押して、コードをクリップボードにコピーしてください。
   ボタンが見つからない場合は「Raw」を押して表示されたページで Ctrl+A（Mac は ⌘+A）→ Ctrl+C（⌘+C）でコピーしてください。
4. Apps Script のタブに戻り、エディタ（Code.gs / コード.gs）の中をクリックして、Ctrl+A（⌘+A）で全選択 → Ctrl+V（⌘+V）で貼り付けてください。
5. 貼り付けたコードの先頭付近に「青年委員 応援カレンダー」、末尾付近に「function jpDate_」があることを確認してください。
6. Ctrl+S（⌘+S）で保存してください。
7. 左側の歯車「プロジェクトの設定」を開き、タイムゾーンが「(GMT+09:00) 日本標準時」または「Asia/Tokyo」になっているか確認してください。違う場合は変更してください。その後、左側の「<>エディタ」に戻ってください。

【手順3：setup を実行する】
1. エディタ上部の関数の選択欄（「実行」ボタンの右）で「setup」を選び、「実行」を押してください。
2. 「承認が必要です」が出たら、ルールどおり作業を止めて私に承認を任せてください。
3. 実行ログに「実行完了」と出たら、スプレッドシートのタブに戻り、「予定」「参加者」「応援メッセージ」の3つのシートができていること、「予定」シートに6件（9/29 行橋〜10/27 博多みなと）が入っていることを確認して報告してください。

【手順4：ウェブアプリとしてデプロイする】
1. Apps Script のタブで、右上の「デプロイ」→「新しいデプロイ」を押してください。
2. 「種類の選択」の歯車アイコンから「ウェブアプリ」を選んでください。
3. 説明：「応援カレンダー」
   次のユーザーとして実行：「自分」
   アクセスできるユーザー：「全員」
   に設定して「デプロイ」を押してください。（承認画面が出たら止めて私に任せてください）
4. 表示された「ウェブアプリ」の URL（https://script.google.com/macros/s/…/exec で終わるもの）をコピーし、チャットに書き出して私に伝えてください。これを【WEBAPP_URL】と呼びます。
5. 新しいタブで【WEBAPP_URL】に「?action=list」を付けて開き、"ok":true と予定のデータ（"events"）が表示されることを確認してください。

【手順5：GitHub のページに URL を設定する】
1. 次のページを開いてください。
   https://github.com/lc100068-cmd/seinen-ouen/edit/main/config.js
2. 「window.SEINEN_API_URL = "";」の行の "" の間に【WEBAPP_URL】を入れてください。
   例：window.SEINEN_API_URL = "https://script.google.com/macros/s/XXXX/exec";
3. 「Commit changes...」を押し、コミットメッセージを「Set Apps Script URL」にして、「Commit directly to the main branch」を選んで確定してください。（確定前に、内容を私に見せて確認を取ってください）

【手順6：GitHub Pages を公開する】
1. https://github.com/lc100068-cmd/seinen-ouen/settings/pages を開いてください。
2. 「Build and deployment」の Source を「Deploy from a branch」、Branch を「main」「/(root)」にして「Save」を押してください。
3. 1〜3分待ってからページを再読み込みし、「Your site is live at …」と表示された URL を私に伝えてください。これを【PAGE_URL】と呼びます。
4. 【PAGE_URL】を開き、「これからの予定」に6件が表示されることを確認してください（表示まで数十秒かかることがあります）。

【手順7：GAS にページの URL を入れて、リマインドを有効にする】
1. Apps Script のタブに戻り、コード上部の
   PAGE_URL: '',
   の '' の間に【PAGE_URL】を入れて、Ctrl+S（⌘+S）で保存してください。
2. 「デプロイ」→「デプロイを管理」→ 鉛筆アイコン（編集）→ バージョンで「新バージョン」を選び →「デプロイ」を押してください（URL は変わりません）。
3. 関数の選択欄で「setupReminderTrigger」を選んで「実行」を押してください。（承認画面が出たら止めて私に任せてください）
4. 左側の時計アイコン「トリガー」を開き、「sendReminders」が「日タイマー」「午後6時〜7時」で登録されていることを確認してください。
5. 関数の選択欄で「testReminderToMe」を選んで「実行」を押してください。自分宛てにテストのリマインドメールが1通届きます。
...
- 【WEBAPP_URL】
- 【PAGE_URL】
- リマインドのトリガーが登録できたかどうか
- 途中でうまくいかなかった点があれば、その内容
```
...
プロンプトの手順2でコピーに失敗したときは、下のコードをすべてコピーして Apps Script に貼り付けてください（`gas/Code.gs` と同じ内容です）。
